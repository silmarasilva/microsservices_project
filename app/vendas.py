...
def add_compra():
    try:
        ...
        if _data and _idCliente and _idCurso and request.method == 'POST':
            ...
            curso = requests.get(f'http://{cursos}/produtos/{_idCurso}', headers = {"Authorization":"Basic c2lsc2lsOjEyMzQ1Njc="})
...
def id_compras (idCliente):
    try:
        ...
        for i in (userRow):
            ...
            # i["data"] = f"{i['data']}"
            # curso.append(c.json())
            c = requests.get(f'http://{cursos}/produtos/{i["idCurso"]}', headers = {"Authorization":"Basic c2lsc2lsOjEyMzQ1Njc="}, verify=False)
            i["data"] = f"{i['data']}"
            curso.append(c.json())
